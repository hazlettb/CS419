...
class appt_db:
	...
	def cancel(self, time, a_fname, a_lname):
		self.cur = self.db.cursor()
		self.cur.execute("UPDATE appointment SET status = 'CANCELLED' WHERE starttime = '"+time+"' AND advisor_fname = %s AND advisor_lname = %s", (a_fname, a_lname))
		self.db.commit()
